...
resource "azurerm_public_ip" "web_ag_publicip" {
  name                = "${local.resource_name_prefix}-web-ag-publicip-${random_string.myrandom.id}"
...
# -----------------------------------
# Locals: Reusable Naming Conventions
# -----------------------------------
locals {
  # Generic
  ...
  request_routing_rule1_name     = "${azurerm_virtual_network.vnet.name}-rrt-1"

  # App1-specific
  backend_address_pool_name_app1 = "${azurerm_virtual_network.vnet.name}-beap-app1"
  http_setting_name_app1         = "${azurerm_virtual_network.vnet.name}-be-hstst-app1"
...
resource "azurerm_application_gateway" "web_ag" {
  name                = "${local.resource_name_prefix}-web-ag-${random_string.myrandom.id}"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
